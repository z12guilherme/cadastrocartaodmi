    ...
    try {
        ...
        const sendToTelegram = async (fileInfo: any) => {
            ...
            if (fileInfo.type === 'document') {
                // Instanciando como File para garantir que o Telegram reconheça o formato PDF
                const fileObj = new File([data], fileInfo.filename, { type: 'application/pdf' });
                formData.append('document', fileObj);
                formData.append('caption', `Contrato Final: ${clienteNome}`);
            } else {
                const fileObj = new File([data], fileInfo.filename, { type: 'image/jpeg' });
        ...
        const telegramPromise = (async () => {
            // Executa os uploads em paralelo para evitar Timeout da Edge Function
            const uploadResults = await Promise.all(filesToBackup.map(file => sendToTelegram(file)));
            return uploadResults.every(success => success === true);
        })();
